...
def make_rpc(balances, submits):
    def fake_rpc(node, method, params=None, timeout=None):
        params = params or {}
        if method == "server_info":
            return {"info": {"server_state": "full", "network_id": 24100}}
        if method == "wallet_propose":
            assert params == {"seed": "PLACEHOLDER_SEED"}
            return {"account_id": FAUCET_ADDRESS}
        if method == "account_info":
            account = params["account"]
            if account not in balances:
                return {"error": "actNotFound"}
            return {"account_data": {"Balance": str(balances[account])}}
        if method == "submit":
            submits.append(params)
            return {"engine_result": "tesSUCCESS", "tx_json": {"hash": "ABC"}}
...
def test_fund_signs_with_genesis_passphrase_and_keeps_reserve(monkeypatch):
    ...
    submits = []
    monkeypatch.setattr(faucet, "rpc", make_rpc({faucet.GENESIS_ACCOUNT: 1_000_000_000}, submits))
...
def test_fund_skips_when_already_funded(monkeypatch):
    ...
    monkeypatch.setattr(faucet, "rpc", make_rpc(balances, submits))
    result = faucet.fund_faucet(inv, "PLACEHOLDER_SEED")
...
def test_verify_reports_low_balance(monkeypatch):
    inv = load_inventory(INVENTORY)
    monkeypatch.setattr(faucet, "rpc", make_rpc({FAUCET_ADDRESS: 5_000_000}, []))
